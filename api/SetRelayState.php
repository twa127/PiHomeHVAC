...
if(isset($_GET['relayname'])) {
        ...
        $query = "SELECT * FROM relays where name = '{$relayname}' LIMIT 1;";
	...
	if(mysqli_num_rows($result) == 0) {
		...
		$node_result = $conn->query($query);
		$node = mysqli_fetch_array($node_result);
			...
			if($status == 0 or $status == 1) {
				if (strpos($type, 'Tasmota') !== false) {
					...
					$update = 0;
                                ...
                                $query = "UPDATE relays SET state = '{$status}', restore_state = '{$status}' WHERE id = '{$id}';";
                                if($conn->query($query)){
                                	$update_error=0;
                                }else{
                                        $update_error=1;
        		...
        		http_response_code(200);
                       	if($row['state'] == 1) {$status = True; $on_off = 'on';} else {$status = False; $on_off = 'off';}
