...
echo "\033[31m";
echo "****************************************************************\n";
echo "*    Boiler Reset Script Version 0.1 Build Date 18/02/2025     *\n";
echo "*             Last Modification Date 18/02/2025                *\n";
...
$boiler_reset_script_txt = 'python3 /var/www/cron/ebus/boiler_reset.py';
$line = "--------------------------------------------------------------------------\n";

echo "\033[36m".date('Y-m-d H:i:s'). "\033[0m - Python Boiler Reset Script Status Check Script Started \n";

// Checking if GPIO Switch script is running
exec("ps ax | grep '$boiler_reset_script_txt' | grep -v grep", $pids);
$nopids = count($pids);
if($nopids==0) { // Script not running
	echo "\033[36m".date('Y-m-d H:i:s'). "\033[0m - Python  Boiler reset Script \033[41mNot Running\033[0m \n";
	echo "\033[36m".date('Y-m-d H:i:s'). "\033[0m - Starting Python Script for Boiler reset\n";
	exec("$boiler_reset_script_txt </dev/null >/dev/null 2>&1 & ");
	exec("ps aux | grep '$boiler_reset_script_txt' | grep -v grep | awk '{ print $2 }' | head -1", $out);
	...
	if($nopids>1) { // Proceed if more than one eBUS script running
		echo "\033[36m".date('Y-m-d H:i:s'). "\033[0m - Multiple Boiler Rest Scripts are Detected \033[41m$nopids\033[0m \n";
		$regex = preg_quote($boiler_reset_script_txt, '/');
		...
		if($nopids-$notpids>1 || $nopids-$notpids==0) { // Proceed if none or more than one script runs
			if($nopids-$notpids>1) { // Proceed if more than one active eBUS script 
				exec("ps -eo s,pid,cmd | grep '$boiler_reset_script_txt' | grep -v grep | awk '{ print $2 }'", $tpids);
				$notpids=$nopids-$notpids;
				echo "\033[36m".date('Y-m-d H:i:s'). "\033[0m - Multiple Active Boiler Rest Script are Running \033[41m$notpids\033[0m \n";
				foreach($tpids as $tpid){
	...
	exec("ps -eo s,pid,cmd | grep '$boiler_reset_script_txt' | grep -v grep | awk '{ print $2 }' | head -1", $out);
...
echo "\033[36m".date('Y-m-d H:i:s'). "\033[0m - Python Boiler Rest Script Status Check Script Ended \n"; 
echo "\033[32m***************************************************************************\033[0m";
echo "\n";
?>
