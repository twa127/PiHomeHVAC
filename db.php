...
if($what=="schedule"){
        ...
        if($opp=="active"){
                ...
                $conn->query($query);
                $query  = "UPDATE schedule_daily_time_zone SET sync = '0', disabled = '".$dis."' WHERE schedule_daily_time_id = '".$wid."'";
                $conn->query($query);
        } elseif ($opp=="delete") {
                if($sch_type <> 2) {
...
if($what=="toggle_relay"){
        ...
        if($opp=="exit"){
                $db_error = 0;
                $query = "SELECT relay_id, relay_child_id, restore_state FROM relays ORDER BY relay_id, relay_child_id DESC;";
                $results = $conn->query($query);
                $count = $results->num_rows;
                if ($count != 0) {
			$n = 0;
                	while ($row = mysqli_fetch_assoc($results)) {
                                $payload = $row['restore_state'];
                ...
                if($db_error == 0){
        	        ...
        	        if ($count != 0) {
			                ...
			                if(!$conn->query($query)){
                                ...
                                if ($db_error == 0) {
                                        $query = "UPDATE relays SET restore_state = state;";
                                        if(!$conn->query($query)){
                                                $db_error = 1;
                                        }
                                }
